---
title: "Cox's PH Model for Cause-Specific Hazard Using R"
---

## Objective

In this document we present how to estimate the the Cox's proportional hazard ratio of cause-specific hazard in R. We focus on the competing risks model where each subject experiences only one out of *k* possible events as depicted in the figure below.
...
We discuss how to implement the method in the `survival` package. 
...
-   `WaitTime` is the waiting time to transplant in days. For illustration purposes, a categorical variable `waitCat` is created as `waitCat = 1` if `WaitTime > 200`, and `waitCat = 0` otherwise.
...
bmt <- haven::read_sas(file.path("../data/bmt.sas7bdat")) %>%
  mutate(
    ...
    TYears = T / 365.25,
    waitCat = ifelse(WaitTime > 200, 1, 0),
    ID = row_number()
  )
```

## Estimating the cause-specific hazard ratio
